...
function collectInputs(defaultTitle, sourceBranch, prefilledValues = {}) {
  ...
  // Get draft status (use prefilled or prompt)
  // Note: --draft and --web cannot be used together in gh CLI
  let draft = prefilledValues.draft;
  const hasWebFlag = prefilledValues.web;

  if (hasWebFlag && draft) {
    console.log('⚠️  Note: --draft cannot be used with --web, creating as regular PR');
    draft = false;
  } else if (!draft && !hasWebFlag) {
    try {
      const draftChoice = execSilent('gum confirm "Create as draft PR?" && echo "yes" || echo "no"');
      draft = draftChoice === 'yes';
    } catch {
      draft = false;
    }
  }
...
async function main() {
  ...
  // Check prerequisites
  // Note: use == null to check for both null and undefined (empty string "" is valid)
  const needsGum = !args.title || args.description == null || !args.base;
